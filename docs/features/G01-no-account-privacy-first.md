...
**Competitor verdict:** Partial gap â€” no-account exists in one *logger* (Nighp), but every wake-window *scheduler* requires an account. "No account, and it actually schedules" is unoccupied.

## Problem / user need
Baby data is intimate â€” sleep, feeding, health, a child's name and birthday. Parents are increasingly wary of handing it over. Yet every major scheduler gates a plan behind a mandatory sign-up, then monetizes or shares the data. A tired parent in a dark nursery should not have to create an account, verify an email, and accept a data policy just to find out when the next nap is.
...
- **Nighp's** logger is local-first and "collects no data" â€” but it doesn't schedule.
...
## Scope â€” MVP
No-account plan flow (birthday + wake time â†’ full plan, no login) + a short, plain-language privacy page stating: no accounts, no data sold, local-first, no AI training (see G04).

## Scope â€” later
Optional opt-in sync for cross-device history; a signed "we can't see your data" architecture note for the methodology page.

## Edge cases & gotchas
- URL state can leak a child's name/birthday if a link is shared publicly â€” warn on share, keep identifying fields optional.
...
## Evidence & citations
Glow â€” [California AG settlement](https://oag.ca.gov/news/press-releases/attorney-general-becerra-announces-landmark-settlement-against-glow-inc-%E2%80%93) Â· [Mozilla Privacy Not Included](https://www.mozillafoundation.org/en/privacynotincluded/glow-nurture-glow-baby/) Â· [Napper Privacy Policy](https://napper.app/privacy/) Â· [Nighp â€” no-account, collects no data](https://apps.apple.com/us/app/baby-tracker-newborn-log/id779656557)

## Effort
Low â€” aligns with existing URL-state architecture.
...
## Success metric
Landing â†’ plan conversion without account; privacy-page engagement; trust mentions in reviews.
